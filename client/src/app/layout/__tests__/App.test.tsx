...
describe('App', () => {
  ...
  it('does not add a locally created activity when persistence fails', async () => {
    const consoleSpy = vi.spyOn(console, 'error').mockImplementation(() => {});
    ...
    await user.type(screen.getByLabelText(/date/i), '2026-11-12T18:30');
    await user.type(screen.getByLabelText(/description/i), 'This should not appear locally');
    await user.type(screen.getByLabelText(/category/i), 'Networking');
    await user.type(screen.getByLabelText(/city/i), 'New York');
    await user.type(screen.getByLabelText(/venue/i), 'Innovation Loft');
    await user.click(screen.getByRole('button', { name: /submit/i }));
    ...
    await waitFor(() => {
      expect(mockedAxios.post).toHaveBeenCalledWith(
        `${getApiBaseUrl()}/api/activities`,
        expect.objectContaining({ title: 'Unsaved Activity' })
      );
    });
