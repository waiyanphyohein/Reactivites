...
public class EditEventHandlerTests : IDisposable
{
    ...
    [Fact]
    public async Task Handle_ExistingEventWithDifferentGroupId_UpdatesEventByEventId()
    {
        // Arrange
        var existingEvent = EventTestData.CreateValidEventWithDifferentGroupId();
        _context.Events.Add(existingEvent);
        await _context.SaveChangesAsync();
        ...
        result.EventName.Should().Be("Updated By EventId");
    }
