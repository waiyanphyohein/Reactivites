    ...
    public MappingProfiles()
    {
        ...
        CreateMap<Event, Event>()
            ...
            .ForAllMembers(opts => opts.Condition((src, dest, srcMember) =>
                srcMember != null &&
                !(srcMember is string str && string.IsNullOrWhiteSpace(str)) &&
                !(srcMember is Guid g && g == Guid.Empty) && // ignore empty Guid values
                !(srcMember is double d && d == 0) &&
                !(srcMember is float f && f == 0f) &&
                !(srcMember is int i && i == 0) &&
                !(srcMember is long l && l == 0) &&
                !(srcMember is decimal m && m == 0m) &&
